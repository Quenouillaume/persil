...
class FilteredComplex:
    ...
    def degree(self, s):
        """
        Check if simplex s is already in the complex, returns the degree if it is, -1 otherwise
        """
        if s in self._degrees_dict:
            return self._degrees_dict[s]
        return -1

    def order(self, s1, d1, s2, d2):
        """
        Return True if s1 with degree d1 is greater than s2 with degree d2
        """
    ...
    def append(self, s, d):
        """
        simplex as a list of vertices, degree.
        Insert so that the order is preserved
